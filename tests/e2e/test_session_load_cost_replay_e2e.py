...
from probe.client import AgentChunk, ProbeClient, ToolCallEvent, UserChunk
...
def _sessions_db() -> Path:
    return Path(tempfile.gettempdir()) / "mustang-e2e-kernel" / ".mustang" / "sessions" / "sessions.db"
            ...
            ConversationMessageEvent(
                ...
                message={
                    ...
                    "content": [
                        ...
                        {"type": "tool_use", "id": "tool-e2e", "name": "Bash", "input": {"command": "pwd"}},
                    ],
                },
                **base,
            ),
    ...
    with sqlite3.connect(db_path) as db:
        ...
        _insert_event(
            ...
            ConversationMessageEvent(
                ...
                timestamp=now + timedelta(milliseconds=1),
                message={"role": "assistant", "content": [{"type": "text", "text": "mixed answer"}]},
                **base,
            ),
        )
        _insert_event(
...
def test_session_load_replays_conversation_history_and_cost(kernel: tuple[int, str]) -> None:
    ...
    async def _run_test() -> None:
        async with ProbeClient(port=port, token=token, request_timeout=_TEST_TIMEOUT) as client:
            ...
            assert any(isinstance(event, UserChunk) and event.text == "loaded question" for event in history)
            assert any(isinstance(event, AgentChunk) and event.text == "loaded answer" for event in history)
            assert any(isinstance(event, ToolCallEvent) and event.tool_call_id == "tool-e2e" for event in history)
...
def test_session_load_recovers_mixed_incomplete_ui_history(kernel: tuple[int, str]) -> None:
    ...
    async def _run_test() -> None:
        async with ProbeClient(port=port, token=token, request_timeout=_TEST_TIMEOUT) as client:
            ...
            history = await client.load_session(session_id)
            assert any(isinstance(event, UserChunk) and event.text == "mixed question" for event in history)
            assert any(isinstance(event, AgentChunk) and event.text == "mixed answer" for event in history)
