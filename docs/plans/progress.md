...
|---|---|---|---|
| 2026-05-12 | Kernel-wide Thinking toggle | Added Kernel-level LLM thinking config, ACP get/set methods routed through Agent Hub to the Primary Runtime, provider capability gating, and CLI `/thinking [on\|off]`. Per-model thinking remains only as deprecated config compatibility. | `uv run --project src/kernel pytest tests/kernel/protocol/test_routing.py tests/kernel/agent_hub/test_agent_hub_transport_c.py tests/kernel/llm/test_llm_manager.py tests/kernel/llm_provider/test_deepseek_provider.py tests/kernel/llm_provider/test_openai_compatible_provider.py tests/kernel/llm_provider/test_anthropic_provider.py -q`; `uv run --project src/kernel pytest -m e2e tests/e2e/test_llm_thinking_config_e2e.py -q`; `bun run src/cli/tests/test_model_service.ts && bun run src/cli/tests/test_input_controller_r4.ts`; `bunx tsc --noEmit --pretty false`; `git diff --check` |
