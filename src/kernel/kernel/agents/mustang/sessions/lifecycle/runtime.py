...
from kernel.core.protocol.interfaces.errors import InvalidRequest, ResourceNotFoundError
from kernel.agents.mustang.sessions._shared.base import _SessionMixinBase
from kernel.agents.mustang.runtime import NullAgentResourceView
from kernel.agents.mustang.sessions.context import AgentContext
from kernel.agents.mustang.sessions.events import KERNEL_VERSION, SessionCreatedEvent
...
class SessionLifecycleMixin(_SessionMixinBase):
    ...
    async def startup(self) -> None:
        ...
        self._agent_context = getattr(self, "_agent_context", None) or AgentContext.primary_compat(
            state_dir=self._module_table.state_dir
        )
        self._agent_resource_view = getattr(
            self,
            "_agent_resource_view",
            NullAgentResourceView(),
        )
        sessions_dir = self._agent_context.sessions_dir
        ...
        from kernel.core.config.section import MutableSection
        from kernel.agents.mustang.orchestrator.config_section import OrchestratorPrefs
    ...
    def _subscribe_llm_config(self) -> None:
        ...
        try:
            from kernel.agents.mustang.llm.config import LLMConfig
    ...
    def _sync_default_model_for_active_sessions(
        ...
        from kernel.agents.mustang.orchestrator.config import OrchestratorConfigPatch
    ...
    async def _close_runtime(self, session: Session, *, quiet: bool) -> None:
        ...
        try:
            from kernel.agents.mustang.tools.builtin.python_tool import shutdown_python_worker
