...
def delete_hash(path: Path) -> None:
    ...
    Called by :meth:`kernel.agents.access.security.ConnectionAuthenticator.clear_password` to
...
def _deserialize(stored_hash: str) -> tuple[int, int, int, bytes, bytes]:
    ...
    ValueError
        ...
        surface of :class:`kernel.agents.access.security.ConnectionAuthenticator`.
    """
