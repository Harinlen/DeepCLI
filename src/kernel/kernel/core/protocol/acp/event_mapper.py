...
``SubAgentStart``              → ``tool_call_update`` + ``_meta: mustang.agent/agentStart``
``SubAgentEnd``                → ``tool_call_update`` + ``_meta: mustang.agent/agentEnd``
...
from kernel.core.protocol.acp.schemas.content import (
    AcpEmbeddedResource,
    AcpImageBlock,
    AcpResourceBlock,
...
from kernel.core.protocol.acp.schemas.enums import AcpToolKind
...
class AcpEventMapper:
    """Concrete :class:`~kernel.core.protocol.interfaces.event_mapper.EventMapper`
    ...
    def _build_notification(
        ...
        # with the agent metadata in namespaced ACP ``_meta``.
