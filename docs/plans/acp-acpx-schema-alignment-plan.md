...
| `model/update` | Not in ACP schema | `_mustang.agent/model/update` |
...
- Update kernel and E2E tests that currently call `model/profile_list`,
  `model/set_current`, `model/update`, `session/rename`, `session/archive`,
  `session/delete`, `session/execute_shell`, `session/execute_python`, or
