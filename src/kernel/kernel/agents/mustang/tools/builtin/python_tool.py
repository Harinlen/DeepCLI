...
def _get_worker(session_id: str, cwd: str) -> _Worker:
    ...
    if worker is not None and worker.process.is_alive():
        return worker
    requests: mp.Queue = _MP_CONTEXT.Queue()
    responses: mp.Queue = _MP_CONTEXT.Queue()
    process = _MP_CONTEXT.Process(target=_worker_main, args=(requests, responses, cwd), daemon=True)
    process.start()
