...
protocol stack via :class:`kernel.agents.access.routes.flags.TransportFlags`,
run the fixed ``recv → decode → dispatch → encode → send`` loop,
handle disconnects.  Protocol decoding and session dispatching
live behind the :class:`kernel.agents.access.routes.stack.ProtocolStack`
...
transport (not in :class:`kernel.agents.access.security.ConnectionAuthenticator`) so
...
from kernel.agents.access.security import AuthError, ConnectionAuthenticator
...
from kernel.agents.access.routes.stack import ProtocolError, ProtocolStack, create_stack
...
async def session_ws(ws: WebSocket) -> None:
    ...
    3. Extract credentials from query params, call
       :meth:`kernel.agents.access.security.ConnectionAuthenticator.authenticate`,
...
def _extract_credentials(
    ...
    :meth:`kernel.agents.access.security.ConnectionAuthenticator.authenticate` and
...
def _format_remote_addr(ws: WebSocket) -> str:
    ...
    :class:`kernel.agents.access.security.AuthContext` should ever crash on missing
