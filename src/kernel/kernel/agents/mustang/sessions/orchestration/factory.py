...
from kernel.agents.mustang.orchestrator import Orchestrator, OrchestratorConfig
...
class SessionOrchestratorFactoryMixin(_SessionMixinBase):
    ...
    def _optional_subsystem(self, module_path: str, class_name: str) -> Any:
        ...
        Args:
            module_path: Dotted module path to import, e.g. ``"kernel.agents.mustang.tools"``.
    ...
    def _make_orchestrator(
        ...
        tool_source = self._optional_subsystem("kernel.agents.mustang.tools", "ToolManager")
        authorizer = self._optional_subsystem("kernel.agents.mustang.tool_authz", "ToolAuthorizer")
        ...
        skills = self._optional_subsystem("kernel.agents.mustang.skills", "SkillManager")
        hooks = self._optional_subsystem("kernel.agents.mustang.hooks", "HookManager")
        memory = self._optional_subsystem("kernel.agents.mustang.memory", "MemoryManager")
        _schedule_manager = self._optional_subsystem("kernel.agents.mustang.schedule", "ScheduleManager")
        _git_manager = self._optional_subsystem("kernel.agents.mustang.git", "GitManager")
        _mcp_manager = self._optional_subsystem("kernel.agents.mustang.mcp", "MCPManager")
        ...
        def _route_agent_message(target_agent_id: str, message: str) -> bool:
            agent_hub = getattr(self._module_table, "agent_hub", None)
            ...
            caller_agent_id = self._agent_context.agent_id
            routed = agent_hub.router.route_message(
                RouterFrame(
                    frame_id=f"send-message-{uuid.uuid4().hex}",
                    kind=RouterFrameKind.AGENT_MESSAGE,
                    source=f"agent:{caller_agent_id}",
                    target=RouterTarget(agent_id=target_agent_id),
                    caller=CallerIdentity(
                        kind=CallerIdentityKind.DURABLE_AGENT,
                        subject_id=f"agent:{caller_agent_id}",
        ...
        deps = OrchestratorDeps(
            ...
            route_agent_message=_route_agent_message,
