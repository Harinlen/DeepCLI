...
Transport is not a :class:`kernel.core.lifecycle.Subsystem` — its
...
right after :meth:`kernel.core.flags.FlagManager.initialize`, mirroring
...
from kernel.agents.access.routes.stack import StackName
...
class TransportFlags(BaseModel):
    ...
    ------
    stack:
        Name of the :class:`kernel.agents.access.routes.stack.ProtocolStack` to
        drive the ``/session`` WebSocket loop.  Typed as
        :data:`~kernel.agents.access.routes.stack.StackName` (a ``Literal``) so
        pydantic rejects unknown names during
        :meth:`kernel.core.flags.FlagManager.register` — a misspelling
    ...
    stack: StackName = Field(
        "acp",
        description=("Registered ProtocolStack name — see kernel.agents.access.routes.stack.create_stack."),
    )
