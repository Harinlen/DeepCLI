...
LLMManager ↔ Provider boundary.  Both ``kernel.agents.mustang.llm`` and
``kernel.agents.mustang.llm_provider`` import from here; neither imports from the other's
types module.

Import rule
-----------
``kernel.agents.mustang.llm_provider.*`` may import from ``kernel.agents.mustang.llm.types``.
``kernel.agents.mustang.llm.*`` may import from ``kernel.agents.mustang.llm_provider.*``.
``kernel.agents.mustang.llm.types`` imports nothing from the kernel.
"""
