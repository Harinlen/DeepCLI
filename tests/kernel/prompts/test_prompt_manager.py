...
def tmp_prompts(tmp_path: Path) -> Path:
    ...
    mod_dir.mkdir()
    (mod_dir / "system.txt").write_text("You are a helpful assistant.", encoding="utf-8")
...
class TestLoad:
    def test_loads_all_txt_files(self, loaded_manager: PromptManager) -> None:
        ...
        assert "orchestrator/system" in keys
...
class TestGet:
    def test_returns_exact_text(self, loaded_manager: PromptManager) -> None:
        assert loaded_manager.get("orchestrator/system") == "You are a helpful assistant."
...
class TestHas:
    def test_returns_true_for_loaded_key(self, loaded_manager: PromptManager) -> None:
        assert loaded_manager.has("orchestrator/system") is True
...
class TestUserOverrides:
    def test_user_dir_overrides_default(self, tmp_path: Path) -> None:
        ...
        (defaults / "orchestrator" / "system.txt").write_text("default text")

        user = tmp_path / "user"
        user.mkdir()
        (user / "orchestrator").mkdir()
        (user / "orchestrator" / "system.txt").write_text("user override")

        pm = PromptManager(defaults_dir=defaults, user_dirs=[user])
        pm.load()
        assert pm.get("orchestrator/system") == "user override"
...
class TestDefaultDirectory:
    ...
    def test_default_prompts_load(self) -> None:
        ...
        assert pm.has("orchestrator/system")
        assert pm.has("orchestrator/first_principles")
