...
from kernel.agents import CallerIdentity, CallerIdentityKind, ManagementCapability
...
class GlobalResourceMonitor:
    """Coordinates global resource writes and revision snapshots."""
    ...
    def write(
        ...
        *,
        ...
        if caller.kind is not CallerIdentityKind.DURABLE_AGENT or caller.agent_id is None:
            raise PermissionError("global resource write requires durable agent identity")
        if capability is not ManagementCapability.GLOBAL_RESOURCE_WRITE:
            raise PermissionError("global resource write capability required")
