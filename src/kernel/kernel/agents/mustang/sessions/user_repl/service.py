...
from kernel.core.protocol.acp.schemas.updates import MustangExecution, MustangExecutionUpdateNotification
...
from kernel.agents.mustang.sessions.events import ConversationMessageEvent
from kernel.agents.mustang.sessions.message_serde import serialize_message
...
from kernel.agents.mustang.tools.types import ToolCallProgress, ToolCallResult, ToolInputError

logger = logging.getLogger("kernel.agents.mustang.sessions.user_repl")
...
class UserReplMixin(_SessionMixinBase):
    ...
    async def _execute_user_tool(
        ...
        *,
        ...
        await self._broadcast_execution_update(
            session,
            MustangExecution(
                phase="start",
    ...
    async def _broadcast_execution_update(
        ...
        """Fan a DeepCLI user-execution extension notification to observers."""
        if not session.senders:
            return
        notification = MustangExecutionUpdateNotification(
            session_id=session.session_id,
            execution=execution,
            meta={"mustang.agent/version": 1},
        )
        dead: list[str] = []
        for conn_id, sender in list(session.senders.items()):
            try:
                ...
                logger.exception(
                    "session=%s conn=%s: user execution notify failed — removing",
                    session.session_id,
                    conn_id,
                )
                dead.append(conn_id)
