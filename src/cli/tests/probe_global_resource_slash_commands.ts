...
await executeBuiltinSlashCommand("/agent list", { ctx });
await executeBuiltinSlashCommand("/agent read worker", { ctx });
await executeBuiltinSlashCommand("/agent create worker /tmp/workspace Worker", { ctx });
await executeBuiltinSlashCommand("/agent bind worker test:chan session-a", { ctx });
...
await executeBuiltinSlashCommand("/agent delete worker --confirm", { ctx });
...
console.log(`agent_management_commands=${countPrefix("_mustang.agent/agents/")}`);
