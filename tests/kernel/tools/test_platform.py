...
from kernel.agents.mustang.tools.platform import (
    has_bash,
    has_powershell,
    is_windows,
    use_powershell_tool,
...
class TestIsWindows:
    def test_true_on_win32(self) -> None:
        with patch("kernel.agents.mustang.tools.platform.sys") as mock_sys:
...
class TestHasPowershell:
    def test_true_when_pwsh_found(self) -> None:
        with patch(
            "kernel.agents.mustang.tools.platform.shutil.which",
    ...
    def test_false_when_neither_found(self) -> None:
        with patch("kernel.agents.mustang.tools.platform.shutil.which", return_value=None):
            assert has_powershell() is False


class TestHasBash:
    def test_true_when_bash_found(self) -> None:
        with patch("kernel.agents.mustang.tools.platform.shutil.which", return_value="/bin/bash"):
...
class TestUsePowershellTool:
    ...
    def test_false_on_non_windows(self) -> None:
        ...
        with patch("kernel.agents.mustang.tools.platform.is_windows", return_value=False):
    ...
    def test_false_when_env_forces_bash(self) -> None:
        ...
        with (
            patch("kernel.agents.mustang.tools.platform.is_windows", return_value=True),
    ...
    def test_true_when_powershell_available(self) -> None:
        ...
        with (
            ...
            patch("kernel.agents.mustang.tools.platform.has_powershell", return_value=True),
        ):
    ...
    def test_false_when_no_powershell_but_bash(self) -> None:
        ...
        with (
            patch("kernel.agents.mustang.tools.platform.is_windows", return_value=True),
            patch.dict("os.environ", {}, clear=True),
            patch("kernel.agents.mustang.tools.platform.has_powershell", return_value=False),
            patch("kernel.agents.mustang.tools.platform.has_bash", return_value=True),
        ):
            assert use_powershell_tool() is False
    ...
    def test_true_when_neither_shell_found(self) -> None:
        ...
        with (
            ...
            patch.dict("os.environ", {}, clear=True),
            patch("kernel.agents.mustang.tools.platform.has_powershell", return_value=False),
            patch("kernel.agents.mustang.tools.platform.has_bash", return_value=False),
        ):
            assert use_powershell_tool() is True
