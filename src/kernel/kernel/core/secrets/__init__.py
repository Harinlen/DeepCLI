...
after :class:`kernel.core.flags.FlagManager` and **before**
:class:`kernel.core.config.ConfigManager`.  ConfigManager may contain
``${secret:name}`` references that need SecretManager to be ready
before YAML values are expanded.

This is **not** a :class:`kernel.core.lifecycle.Subsystem` subclass — it
is a bootstrap service with a dedicated typed slot on
:class:`kernel.agents.mustang.module_table.KernelModuleTable`, same as FlagManager
and ConfigManager.
...
from kernel.core.secrets.types import (
    OAuthToken,
    SecretDatabaseError,
    SecretNotFoundError,
)
